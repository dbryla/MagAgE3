...
final class EventsObserver {
	...
	private static final Logger log = LoggerFactory
			.getLogger(EventsObserver.class);
	...
	private static void addAnotherListeners() {
		...
		client.getLifecycleService().addLifecycleListener(
				new LifecycleListenerImpl());
		client.getPartitionService().addPartitionLostListener(
				new PartitionLostListenerImpl());
	...
	private static void collectInitalData() {
		...
		for (DistributedObject instance : instances) {
			...
			case "hz:impl:mapService":
				...
				case "worker/state":
					((IMap<String, ComputationState>) instance).entrySet()
							.forEach(
									entry -> ClusterManager.addWorkerState(
											entry.getKey(), entry.getValue()));
					((IMap<String, ComputationState>) instance)
							.addEntryListener(new WorkerStateListener(), true);
					break;
				case "status/map":
					((IMap<String, Status>) instance).entrySet().forEach(
							entry -> ClusterManager.addNodeStatus(
									entry.getKey(), entry.getValue()));
					((IMap<String, Status>) instance).addEntryListener(
							new NodeStatusListener(), true);
					break;
				case "discovery/members":
					((IMap<String, NodeDescriptor>) instance).entrySet()
							.forEach(
									entry -> ClusterManager.addNodeDescriptor(
											entry.getKey(), entry.getValue()));
					((IMap<String, Status>) instance).addEntryListener(
							new NodeDescriptorListener(), true);
					break;
				case "topology/config":
					ClusterManager.setMaster(((IMap<String, String>) instance)
							.get("master"));
					ClusterManager
							.setTopologyGraph(((IMap<String, UnmodifiableDirectedGraph>) instance)
									.get("topologyGraph"));
					ClusterManager
							.setTopologyType(((IMap<String, String>) instance)
									.get("topologyType"));
					((IMap<String, ?>) instance).addEntryListener(
							new TopologyConfigListener(), true);
					break;
				default:
					log.info("Received unknown map: "
							+ ((IMap<?, ?>) instance).getName());
				}
				break;
			case "hz:impl:topicService":
				((ITopic<?>) instance)
						.addMessageListener(new MessageListenerImpl());
				break;
			case "hz:impl:replicatedMapService":
				((ReplicatedMap<?, ?>) instance)
						.addEntryListener(new WorkerConfigListener());
				break;
			default:
				log.info("Received unknown distributed object: "
